...
export default function Events({ lang = 'en' }) {
  ...
  // If highlights.json is not provided, allow using events marked as `featured` in CMS.
  const featuredHighlights = useMemo(() => {
    const todayIso = toISODate(new Date())
    return events
      .filter((e) => e?.featured)
      .filter((e) => e?.date && e.date <= todayIso) // "past" by default
  ...
  const highlightsToShow = useMemo(() => {
    // If CMS highlights.json is present and not just the fallback, show it.
    const usingFallback = highlights === FALLBACK_HIGHLIGHTS
    if (!usingFallback && Array.isArray(highlights) && highlights.length) return highlights
    if (featuredHighlights.length) return featuredHighlights
    return highlights
      ...
      <section id="events" className="section">
        ...
        <div className="carouselWrap">
          ...
          <InfiniteSeamlessCarousel
              ...
              secondsPerLoop={16}
              pauseOnHover
              items={highlightsToShow}
              renderItem={(h) => {
                const mapped = highlightMap?.[h.key]
                const src = mapped || h.image || ''
                const goTo = () => {
                  if (h?.eventSlug) navigate(`/events/${h.eventSlug}`)
                  else if (h?.href) window.open(h.href, '_blank', 'noreferrer')
                }
                const clickable = !!(h?.eventSlug || h?.href)
                return (
                    <div className={`slide ${clickable ? 'clickable' : ''}`} key={h.key} onClick={clickable ? goTo : undefined} role={clickable ? 'button' : undefined} tabIndex={clickable ? 0 : undefined}>
