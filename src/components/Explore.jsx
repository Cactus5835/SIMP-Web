...
export default function Explore({ lang = 'en', showInstagramEmbed = false }) {
  ...
  const links = (data?.links?.length ? data.links : FALLBACK.links).filter(Boolean).slice(0, 5)
      ...
      <div className="carousel socialCarousel" aria-label="Social media links">
        {links.map((x, i) => (
          <a
            ...
            className="linkSlide"
            href={x.url || '#'}
            target={x.url && /^https?:\/\//i.test(x.url) ? '_blank' : undefined}
            rel={x.url && /^https?:\/\//i.test(x.url) ? 'noreferrer' : undefined}
            onClick={(e) => {
